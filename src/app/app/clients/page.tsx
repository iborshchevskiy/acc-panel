    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <form action={addClient} className="flex flex-wrap gap-3 rounded-xl p-4"
        ...
        <button type="submit" className="h-9 rounded-md px-4 text-sm font-medium transition-colors"
          style={{ backgroundColor: "#10b981", color: "#0d1117" }}>Add client</button>
      </form>
      ...
      {rows.length === 0 ? (
        ...
        <div className="overflow-hidden rounded-xl" style={{ border: "1px solid #1e2432" }}>
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.map((c, i) => (
                <tr key={c.id} style={{ backgroundColor: "#0d1117", borderBottom: i < rows.length - 1 ? "1px solid #1e2432" : "none" }}>
