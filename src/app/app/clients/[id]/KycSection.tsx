  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div
        ...
        onDrop={onDrop}
        className="relative flex flex-col items-center justify-center gap-1.5 rounded-lg cursor-pointer transition-all"
        style={{
          height: 72,
      ...
      >
        ...
        {busy ? (
          <span className="text-xs" style={{ color: "var(--text-4)" }}>Uploading…</span>
        ) : (
          <>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" style={{ color: "var(--text-4)" }}>
          ...
          {zoneDocs.map((doc) => (
            <div key={doc.id} className="flex items-center gap-2 rounded-md px-2.5 py-1.5"
              ...
              <button type="button" onClick={() => handleDownload(doc.id)}
                className="text-[10px] transition-opacity hover:opacity-70 shrink-0"
                style={{ color: "var(--accent)" }} title="Download">
                ↓
              </button>
              <button type="button" onClick={() => handleDelete(doc.id)}
                className="text-[10px] transition-opacity hover:opacity-70 shrink-0"
                style={{ color: "var(--text-4)" }} title="Delete">
                ×
              </button>
            </div>
          ))}
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ border: "1px solid var(--inner-border)" }}>
      ...
      <div className="p-4" style={{ backgroundColor: "var(--surface)" }}>
        ...
        {/* Document uploads */}
        <div className="mt-5 pt-4 grid grid-cols-3 gap-4" style={{ borderTop: "1px solid var(--inner-border)" }}>
