...
// `buildPeriodBuckets` and `buildSpreadAnalysis` previously lived here as
// in-memory helpers operating on AnalyticsTxRow shape. Both were superseded
// by SQL-aggregated queries on the analytics page (faster, doesn't load the
// full transaction history into Node memory) and removed in the audit-A pass
// after a grep confirmed zero callers.
